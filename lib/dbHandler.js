...
function storeSubscription(mapId, event, cb) {
    if(event && event.feature && event.action){
        event.feature.lastAction = event.action;
    }
    if(event && event.user){
    ...
    couchdb.insert(mapId+'-subs', event.feature, event.fid, function(err, res) {
        if (err) {
            cb(err);
        } else {
            storeAction(mapId, event, res);
...
function deleteSubscription(mapId, event) {
    couchdb.delete(mapId+'-subs', event.feature, event.fid, function(err, res) {
        if (err) {
            console.log(err);
        } else {
            storeAction(mapId+'-subs', event, res);
        }
    });
}
...
module.exports.saveSubscription = function(mapId, event) {
    if (mapId && event && event.feature && event.fid && event.action) {
        if (event.action === 'deleted subscription') {
            deleteSubscription(mapId+'-subs', event);
        } else {
            storeSubscription(mapId+'-subs', event, function(err, res){
                //TODO errorhandling!
                if(err) console.log(err);
                //else console.log(res);
            });
